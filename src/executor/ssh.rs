...
impl SshExecutor {
    ...
    pub fn execute(server: &ManagedServer, command: &str) -> Result<String, String> {
        ...
        let tcp = TcpStream::connect(&address).map_err(|e| format!("Failed to connect to {}: {}", address, e))?;
        ...
        if !sess.authenticated() {
            return Err("Authentication failed: Unable to authenticate via Agent, Key, or Password.".to_string());
        }

        // Create Channel and Execute Command
        let mut channel = sess.channel_session().map_err(|e| format!("Failed to create channel: {}", e))?;
        ...
        if exit_status != 0 {
             // You might want to return stderr here or include it in the error
             return Err(format!("Command exited with status {}. Output: {}", exit_status, s));
        }
